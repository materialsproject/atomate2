...
One common task encountered in almost any materials science calculation is writing calculation input files to disk so they can be executed by the underlying software (e.g., VASP, Q-Chem, CP2K, etc.). This is preferably done via a `pymatgen` `InputSet` class. `InputSet` is essentially a dict-like container that specifies the files that need to be written, and their contents. Similarly to the way that `Maker` classes generate `Flow`s, `InputSet`s are most easily created by `InputGenerator` classes. `InputGenerator` classes have a method `get_input_set()` that typically takes atomic coordinates (e.g., a `Structure` or `Molecule` object) and produce an `InputSet`, e.g.,
...
`pymatgen` already contains `InputSet` for many common codes, so when developing a workflow `Maker` it is convenient to use the `InputGenerator` / `InputSet` to prepare your files. This is done in `atomate2` by making the `InputGenerator` a class parameter, and writing the input files inside a `@job`-decorated function (rather than directly in `make()`), since it is the individual `Job` that actually gets executed, potentially on a remote compute resource.
...
Putting this together, a `Maker` typically defines a single `@job`-decorated `make()` method that (1) writes the input files, (2) runs the underlying code, and (3) parses the output directory into a `TaskDocument`, which is returned as the `Job`'s output via a `Response`:
