...
from atomate2.common.schemas.neb import NebResult, NebPathwayResult
...
def test_approx_neb_flow(mock_vasp, clean_dir, vasp_test_dir):
    ...
    fake_run_vasp_kwargs = {
        key : {"incar_exclude": ["IBRION","ALGO"]} # updated this to generate flow test data more quickly
    ...
    assert all(
        len(getattr(output["collate_results"].hops["3+0"],k)) == 5 # two failed image calcs
        for k in ("energies","images")
    )
    assert all(
        len(getattr(output["collate_results"].hops["3+1"],k)) == 6 # one failed image calc
        for k in ("energies","images")
    )
    ...
    assert all(
        output["collate_results"].max_barriers[k] == pytest.approx(
            max(
    ...
    assert all(
        getattr(output["collate_results"],f"{direction}_barriers")[k] == pytest.approx(
            getattr(ref_results,f"{direction}_barriers")[k]
        )
