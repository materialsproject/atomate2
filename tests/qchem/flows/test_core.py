...
from atomate2.qchem.flows.core import FrequencyOptMaker, FrequencyOptFlatteningMaker
...
def test_frequency_opt_maker(mock_qchem, clean_dir, qchem_test_dir, h2o_molecule):
    ...
    ref_paths = {
        "Geometry Optimization": Path(qchem_test_dir) / "ffopt" / "geometry_optimization",
    ...
    print(output["Geometry Optimization"].output)

    ref_total_energy = -76.346601
    assert output["Geometry Optimization"].output.final_energy == pytest.approx(ref_total_energy, rel=1e-6)

    assert output["Frequency Analysis"].output.final_energy == pytest.approx(ref_total_energy, rel=1e-6)
    ref_freq = [1587.39, 3864.9, 3969.87]
    assert all(
        freq == pytest.approx(ref_freq[i],abs=1e-2) for i, freq in enumerate(output["Frequency Analysis"].output.frequencies)
    )
    assert output["Geometry Optimization"].output.optimized_molecule == output["Frequency Analysis"].output.initial_molecule
    assert output["Frequency Analysis"].output.optimized_molecule is None

def test_frequency_opt_flattening_maker(mock_qchem, clean_dir, qchem_test_dir, h2o_molecule):
    ...
    assert output["Frequency Analysis 1"].output.final_energy == pytest.approx(ref_total_energy, rel=1e-6)
    ref_freq = [1587.39, 3864.9, 3969.87]
    assert all(
        freq == pytest.approx(ref_freq[i],abs=1e-2) for i, freq in enumerate(output["Frequency Analysis 1"].output.frequencies)
    )
    assert output["Geometry Optimization"].output.optimized_molecule == output["Frequency Analysis 1"].output.initial_molecule
