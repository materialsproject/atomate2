...
from atomate2.ase.md import LennardJonesMDMaker, GFNxTBMDMaker
...
def test_ase_nvt_maker(calculator_name, lj_fcc_ne_pars, fcc_ne_structure):
    ...
    assert output.output.energy_per_atom == pytest.approx(reference_energies[calculator_name])
...
def test_ase_npt_maker(calculator_name, lj_fcc_ne_pars, fcc_ne_structure):
    ...
    md_job = name_to_maker[calculator_name](
        ...
        task_document_kwargs = {
            ...
            "ionic_step_data": ("energy","stress"),  # energy is required in ionic steps
        }
    ).make(structure)
