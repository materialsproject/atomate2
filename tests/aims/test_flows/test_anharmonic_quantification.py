...
def test_atom_resolved_anharmonicity(nacl, tmp_path, mock_aims, species_dir):
    # mapping from job name to directory containing test files
    ref_paths = {
    ...
    responses = run_locally(flow, create_folders=True, ensure_success=True)
    os.chdir(cwd)
    nacl_sigma_dict = responses[flow.job_uuids[-1]][1].output.atom_resolved_sigma_A
    assert np.round(nacl_sigma_dict["Na"], 3) == 0.076
    assert np.round(nacl_sigma_dict["Cl"], 3) == 0.072
