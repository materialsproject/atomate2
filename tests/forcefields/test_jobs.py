...
def test_chgnet_relax_maker(si_structure, relax_cell: bool):
    ...
    if relax_cell:
        assert output1.output.energy == approx(-10.62461, rel=1e-4)
        assert output1.output.ionic_steps[-1].magmoms[0] == approx(0.002964, rel=1e-4)
    else:
...
def test_mace_relax_maker(si_structure, test_dir, model, relax_cell: bool):
    ...
    assert isinstance(output1, ForceFieldTaskDocument)
    if relax_cell:
        assert output1.output.energy == approx(-0.071052, rel=1e-4)
