...
class InputSet(MSONable, MutableMapping):
    ...
    def __init__(
        self,
        inputs: Dict[Union[str, Path], Union[str, InputFile]] = {},  # noqa: B006
        **kwargs,
    ):
        """
        ...
        Args:
            inputs: The core mapping of filename: file contents that defines the
                InputSet data. This should be a dict where keys are filenames and
                values are InputFile objects or strings representing the entire
                contents of the file. If a value is not an InputFile object nor
                a str, but has a __str__ method, this str representation
                of the object will be written to the corresponding file. This
    ...
    def write_input(
        ...
        if zip_inputs:
            ...
            with ZipFile(zipfilename, "w") as zip:
                for fname, _contents in self.inputs.items():
                    file = path / fname
                    try:
                        zip.write(file)
                        os.remove(file)
