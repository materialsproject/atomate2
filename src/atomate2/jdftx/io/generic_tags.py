...
def flatten_list(tag, list_of_lists):
    ...
    while all([isinstance(x, list) for x in list_of_lists]):
...
class AbstractTag(ClassPrintFormatter, ABC):
    ...
    @abstractmethod
    def validate_value_type(self, value) -> bool:
        pass

    def _validate_value_type(self, type_check, tag, value, try_auto_type_fix: bool = False) -> bool:
        if self.can_repeat:
        ...
        if not is_valid and try_auto_type_fix:
            try:
                ...
                #print('                 done!', value)
                _, is_valid, _ = self._validate_value_type(type_check, tag, value)
                #print('               now valid', is_valid)
            except:
                warnings.warn(f'Could not fix the typing for {tag} {value}!')
        return tag, is_valid, value

    def _validate_repeat(self, tag, value):
        if not isinstance(value, list):
            raise ValueError(f'The {tag} tag can repeat but is not a list: {value}')

    @abstractmethod
    def read(self, value_str: str):
        pass
...
class BoolTag(AbstractTag):
    _TF_options = {'read': {'yes': True, 'no': False},
...
class TagContainer(AbstractTag):
    ...
    subtags: dict = None
    linebreak_Nth_entry: int = None  #handles special formatting for matrix tags, e.g. lattice tag
    ...
    def _validate_single_entry(self, value, try_auto_type_fix: bool = False):
        ...
        for subtag, subtag_value in value.items():
            ...
            #print('        subtag', subtag, subtag_value, subtag_object)
            tags, checks, subtag_value = subtag_object.validate_value_type(subtag, subtag_value, try_auto_type_fix = try_auto_type_fix)
            #print('    results', tags, checks, subtag_value)
            if try_auto_type_fix:
                updated_value[subtag] = subtag_value
                #print('         updated subtag_value', subtag_value)
        ...
        #print('-----------> type_checks', types_checks)
    ...
    def validate_value_type(self, tag, value, try_auto_type_fix: bool = False) -> bool:
        ...
        if self.can_repeat:
            self._validate_repeat(tag, value)
            results = [self._validate_single_entry(x, try_auto_type_fix = try_auto_type_fix) for x in value]
    ...
    def read(self, tag: str, value: str) -> dict:
        value = value.split()
        if tag == 'ion':
            special_constraints = [x in ['HyperPlane', 'Linear', 'None', 'Planar'] for x in value]
            if any(special_constraints):
                value = value[:special_constraints.index(True)]
                warnings.warn('Found special constraints reading an \'ion\' tag, these were dropped; reading them has not been implemented!')
        ...
        for subtag, subtag_type in self.subtags.items():
            ...
            if len(value) == 0:
                break
            if subtag in tempdict or subtag_type.write_tagname:  #this tag has already been read or requires a tagname keyword to be present
                continue
        ...
        for subtag, subtag_type in self.subtags.items():
            if not subtag_type.optional and not subtag in subdict:
                raise ValueError(f'The {subtag} tag is not optional but was not populated during the read!')
        if len(value) > 0:
            raise ValueError(f'Something is wrong in the JDFTXInfile formatting, some values were not processed: {value}')
        return subdict
    ...
    def write(self, tag: str, value):
        ...
        for subtag, subvalue in value.items():
            ...
            if self.subtags[subtag].can_repeat and isinstance(subvalue, list):
                ...
                print_str = [self.subtags[subtag].write(subtag, entry) for entry in subvalue]
    ...
    def _make_list(self, value):
        ...
        for subtag, subtag_value in value.items():
            ...
            if subtag_type.allow_list_representation:
                ...
                subtag_value = subtag_type.get_list_representation(subtag, value[subtag])  #recursive list generation
    ...
    def _check_for_mixed_nesting(tag, value):
        if any([isinstance(x, dict) or isinstance(x, list) for x in value]):
    ...
    def _determine_format_option(self, tag, value, try_auto_type_fix: bool = False):
        for i, format_option in enumerate(self.format_options):
            try:
                ...
                _, is_tag_valid, value = format_option.validate_value_type(tag, value, try_auto_type_fix = try_auto_type_fix)
