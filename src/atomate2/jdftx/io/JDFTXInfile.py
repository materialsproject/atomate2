...
class JDFTXInfile(dict, MSONable):
    ...
    @classmethod
    def from_file(cls, filename: PathLike, dont_require_structure: bool = False, sort_tags: bool = True) -> Self:
        ...
        with zopen(filename, mode="rt") as file:
            return cls.from_str(file.read(), dont_require_structure = dont_require_structure, sort_tags = sort_tags)
    ...
    def _preprocess_line(line):
        ...
        if tag in __WANNIER_TAGS__:
            raise ValueError('Wannier functionality has not been added!')
        if not tag in __TAG_LIST__:
            raise ValueError(f'The {tag} tag in {line} is not in MASTER_TAG_LIST and is not a comment, something is wrong with this input data!')
        ...
        if len(line) == 2:
            ...
            raise ValueError(f'The len(line.split(maxsplit=1)) of {line} should never not be 1 or 2')
    ...
    def from_str(cls, string: str, dont_require_structure: bool = False, sort_tags: bool = True) -> Self:
        ...
        if not dont_require_structure and not 'lattice' in params and not 'ion' in params and not 'ion-species' in params:
    ...
    def to_JDFTXStructure(cls, JDFTXInfile, sort_structure: bool = False):
        ...
        return JDFTXStructure._from_JDFTXInfile(JDFTXInfile, sort_structure = sort_structure)
    ...
    def to_pmg_structure(cls, JDFTXInfile, sort_structure: bool = False):
        ...
        #use dict representation so it's easy to get the right column for moveScale, rather than checking for velocities
        JDFTXInfile = cls.get_dict_representation(JDFTXInfile)
        return JDFTXStructure._from_JDFTXInfile(JDFTXInfile, sort_structure = sort_structure).structure
    ...
    def _needs_conversion(conversion, value):
        ...
        if isinstance(value, dict):  #value is like {'subtag': 'subtag_value'}
            return flag
        elif all([isinstance(x, dict) for x in value]):  #value is like [{'subtag': 'subtag_value'}, {'subtag': 'subtag_value'}]
            return flag
        else:
            return not flag
    ...
    def validate_tags(self, try_auto_type_fix: bool = False, error_on_failed_fix: bool = True,
                      return_list_rep: bool = False):
        for tag in self:
            tag_object = get_tag_object(tag)
            checked_tags, is_tag_valid, value = tag_object.validate_value_type(tag, self[tag], try_auto_type_fix = try_auto_type_fix)
            ...
            if return_list_rep:
                if tag_object.allow_list_representation:  #converts newly formatted tag into list repr
...
class JDFTXStructure(MSONable):
    ...
    def _from_JDFTXInfile(cls, JDFTXInfile, sort_structure: bool = False):
        ...
        struct = Structure(lattice, atomic_symbols, coords, to_unit_cell = False,
                           validate_proximity = False, coords_are_cartesian = coords_are_cartesian)
    ...
    def from_dict(cls, params: dict) -> Self:
        ...
        Returns:
            JDFTXStructure
        """
        return cls(Structure.from_dict(params["structure"]),
