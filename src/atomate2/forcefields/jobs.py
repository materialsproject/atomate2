...
class CHGNetRelaxMaker(Maker):
    ...
    keep_info : list
        Which information from the relaxation trajectory to save using the :obj:`.FFStructureRelaxDocument.from_chgnet_result`.
    ...
    keep_info: list = field(
        default_factory=lambda: [
            ...
            'atom_positions',
            'cells'
        ] # NOTE: the 'atoms' field will always be removed later because it is not serializable (as of May 2023)
    )
    ...
    def make(self, structure: Structure):
        ...
        result = relaxer.relax(structure, relax_cell=self.relax_cell, **self.relax_kwargs)
