...
class InputDoc(BaseModel):
    structure: Structure = Field(
        None, description="The inputted structure."
    )
    relax_cell: bool = Field(
        None, description="Whether the cell shape/volume was allowed to change during relaxation."
    )
    relax_kwargs: dict = Field(
        None, description="Keyword arguments that were passed to :obj:`StructOptimizer.relax`."
    )
    optimizer_kwargs: dict = Field(
        None, description="Keyword arguments that were passed to :obj:`StructOptimizer`."
    )
...
class OutputDoc(BaseModel):
    structure: Structure = Field(
        None, description="The final, relaxed structure."
    )
    energy: float = Field(
        None, description="Total energy in units of eV"
    )
    energy_per_atom: float = Field(
        None, description="Energy per atom of the final relaxed structure in units of eV/atom"
    )
    forces: List[List[float]] = Field(
        None, description="The force on each atom in units of eV/A^2 for the final structure."
    )
    stress: List[float] = Field(
        None, description="The stress on the cell in units of kbar (in Voigt notation)."
    )
    # NOTE: units for stresses were converted to kbar (* -10 from standard output) to comply with MP convention
...
class FFStructureRelaxDocument(BaseModel):
    ...
    MD_potential: str = Field(
        None, description="name of the interatomic potential used for relaxation.",
    )
    MD_potential_version: str = Field(
        None, description="version of the interatomic potential used for relaxation.",
    )
    ...
    def from_chgnet_result(
        ...
        keep_info : list
            List of which pieces of information from the trajectory is saved in this FFStructureRelaxDocument.
        """
        ...
        if 'atoms' in keep_info:
            warning_msg = """
                WARNING: `Atoms` objects can't be serialized (as of May 2023) and thus are automatically removed.
                """
            logging.warning(warning_msg)
            trajectory_to_save.pop('atoms') # can't serialize `AseAtoms` objects, so remove them from trajectory.
