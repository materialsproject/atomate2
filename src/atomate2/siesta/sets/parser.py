...
class SiestaOutChunk:
    ...
    def search_for_all(
        self, key: str, line_start: int = 0, line_end: int = -1
    ) -> list[int]:
        """
...
class SiestaOutHeaderChunk(SiestaOutChunk):
    ...
    def commit_hash(self) -> str:
        ...
        if line_start == LINE_NOT_FOUND:
            raise SiestaParseError(
                "This file does not appear to be an siesta-output file"
            )
    ...
    def initial_structure(self) -> Structure | Molecule:
        ...
        the calculation.
        """
        logger.info("SiestaOutHeaderChunk.initial_structure()")
    ...
    def _parse_initial_charges_and_moments(self) -> None:
        ...
        line_start = self.reverse_search_for(
            ["Initial charges", "Initial moments and charges"]
        )
        if line_start != LINE_NOT_FOUND:
            line_start += 2
    ...
    def _parse_k_points(self) -> None:
        ...
        if LINE_NOT_FOUND in {line_start, line_end} or (
            line_end - line_start != n_kpts
        ):
            self._cache |= {"k_points": None, "k_point_weights": None}
            return
...
class SiestaOutCalcChunk(SiestaOutChunk):
    ...
    def __init__(self, lines: list[str], header: SiestaOutHeaderChunk) -> None:
        ...
        """
        logger.info("SiestaOutCalcChunk.__init__()")
    ...
    def _parse_structure(self) -> Structure | Molecule:
        ...
        properties = {
            prop: results[prop] for prop in results if prop not in site_prop_keys
        }
    ...
    def _parse_lattice_atom_pos(
        ...
        """
        logger.info("SiestaOutCalcChunk._parse_lattice_atom_pos()")
        lattice_vectors = []
        ...
        if line_start == LINE_NOT_FOUND:
            ...
            velocities = list(
                self.initial_structure.site_properties.get("velocity", [])
            )
            lattice = self.initial_lattice
...
def get_siesta_out_chunks(
    content: str | TextIOWrapper, header_chunk: SiestaOutHeaderChunk
) -> Generator:
    """
...
def check_convergence(
    chunks: list[SiestaOutCalcChunk], non_convergence_ok: bool = False
) -> bool:
    """
...
def read_siesta_output_structure(
    ...
    Reads the final or specified geometry from a SIESTA calculation using
    the XV file format through the sisl library.

    Parameters
    ----------
    filename : str | Path
        Path to the SIESTA XV file (typically 'siesta.XV')
    index : int | slice, default=-1
        Structure index to read (currently only -1/final structure supported)
    non_convergence_ok : bool, default=False
        Whether to allow reading from non-converged calculations (currently unused)

    Returns
    -------
    Structure | Molecule
        Pymatgen Structure or Molecule object containing the atomic positions,
        lattice vectors, and species information

    Notes
    -----
    Currently only returns the final structure regardless of the index parameter.
    Full trajectory support is planned for future implementation.
    """
    logger.info("read_siesta_output_structure()")
