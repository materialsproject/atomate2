...
def recreate(
    ...
    r"""Recreate job with modified FDF parameters (SAFE method).
    ...
    console.print(
        f"\n[bold cyan]Recreating job {job_id} "
        f"from project '{project_name}'[/bold cyan]\n"
    )
...
def _generate_recreation_script(
    ...
    job_index: int,
    project_name: str,
    original_params: dict,  # noqa: ARG001 kept for caller API symmetry
    ...
    # Long hint line interpolated to keep source under the line-length limit
    # while preserving the generated script's output verbatim.
    hint_line = (
        "Please update the structure_file path or "
        "create the structure programmatically."
    )
