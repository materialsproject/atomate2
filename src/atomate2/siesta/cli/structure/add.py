...
def add(
    ...
    align_to: str,
    output: str,
    format: str,  # noqa: A002 Click option name (--format)
    show_added: bool,
) -> None:
    ...
    --------
        ...
        atomate2siesta-structure add surface.cif --molecule CO \
            --position 0.5,0.5,0.9 --fractional

        # Add with rotation (Euler angles)
        atomate2siesta-structure add surface.cif --molecule H2O --on-top \
            --rotate 0,45,90
        ...
        atomate2siesta-structure add surface.cif --molecule-file benzene.xyz \
            --position 0,0,3.0
    """
    try:
        # Validate options
        if not any([element, molecule, molecule_file]):
            console.print(
                "[red]Error: Must specify one of --element, "
                "--molecule, or --molecule-file[/red]"
            )
            raise click.Abort  # noqa: TRY301 friendly CLI abort

        if sum([bool(element), bool(molecule), bool(molecule_file)]) > 1:
            console.print(
                "[red]Error: Can only use one of --element, "
                "--molecule, or --molecule-file[/red]"
            )
            raise click.Abort  # noqa: TRY301 friendly CLI abort
        ...
        if sum([bool(position), on_top, on_bottom]) != 1:
            console.print(
                "[red]Error: Must specify exactly one of --position, "
                "--on-top, or --on-bottom[/red]"
            )
            raise click.Abort  # noqa: TRY301 friendly CLI abort
        ...
        if on_top or on_bottom:
            ...
            console.print(
                f"  Position: ({cart_position[0]:.3f}, "
                f"{cart_position[1]:.3f}, {cart_position[2]:.3f}) Å"
            )
            ...
            if fractional:
                ...
                console.print(
                    f"  Cartesian: ({cart_position[0]:.3f}, "
                    f"{cart_position[1]:.3f}, {cart_position[2]:.3f}) Å"
                )
            else:
                cart_position = np.array(coords)
                console.print(
                    f"\n[yellow]Adding at Cartesian position: ({coords[0]:.3f}, "
                    f"{coords[1]:.3f}, {coords[2]:.3f}) Å[/yellow]"
                )
        ...
        for species, coord in zip(new_species, final_coords, strict=False):
...
def _apply_orientation(
    ...
    rotate: str,
    align_to: str,
    species: list,  # noqa: ARG001 kept for signature symmetry
) -> np.ndarray:
    """Apply rotation/alignment to molecule coordinates."""
    coords = np.array(coords)

    # Align to axis if requested
    if align_to:  # noqa: SIM102 kept nested for alignment logic clarity
        ...
        if len(coords) > 1:
            ...
            if np.linalg.norm(rotation_axis) > 1e-6:
                ...
                console.print(
                    f"[dim]Aligned molecule to {align_to}-axis "
                    f"({np.degrees(angle):.1f}° rotation)[/dim]"
                )
    ...
    if rotate:
        try:
            ...
            Rz1 = np.array(  # noqa: N806 rotation matrix
                [
            ...
            Ry = np.array(  # noqa: N806 rotation matrix
                [
            ...
            Rz2 = np.array(  # noqa: N806 rotation matrix
                [
            ...
            R = Rz2 @ Ry @ Rz1  # noqa: N806 rotation matrix
            center = coords.mean(axis=0)
            coords = (coords - center) @ R.T + center

            console.print(
                f"[dim]Rotated molecule (α={angles[0]:.1f}°, β={angles[1]:.1f}°, γ={angles[2]:.1f}°)[/dim]"  # noqa: RUF001, E501 Greek Euler-angle symbols
            )
...
def _rotate_coords(coords: np.ndarray, axis: np.ndarray, angle: float) -> np.ndarray:
...
def _display_added_atoms(species: list, coords: list) -> None:
    """Display added atoms."""
    ...
    for sp, coord in zip(species, coords, strict=False):
